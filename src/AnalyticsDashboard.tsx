...
type AnalyticsView = 'summary' | 'online' | 'today' | 'countries' | 'visitors' | 'downloads' | 'activity' | 'gps' | 'reports';
            ...
            <table className="analytics-table">
              <thead>
                <tr>
                  ...
                  <th className="analytics-del-col">{ar ? 'حذف' : 'Del'}</th>
                  <th>{ar ? 'النوع' : 'Type'}</th><th>{ar ? 'التفاصيل' : 'Details'}</th><th>IP</th><th>{ar ? 'المدينة / البلد' : 'City / country'}</th><th>{ar ? 'الوقت' : 'Time'}</th>
                </tr>
              </thead>
              <tbody>
        ...
        <div style={{ marginTop: 18 }}>
          ...
          {(stats?.gpsVisitors?.length ?? 0) > 0 ? (
            ...
            <p className="analytics-muted" style={{ textAlign: 'center', padding: 24 }}>
              {ar ? 'لا توجد موافقات GPS محفوظة بعد — تظهر عند موافقة الزائر من قسم الزراعة (تحديد موقعي)' : 'No GPS consents saved yet'}
            </p>
          )}
        </div>
      )}
