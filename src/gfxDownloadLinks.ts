...
export function applyGfxDownloadLinks(item: GfxProjectItem, links: GfxDownloadLink[]): GfxProjectItem {
  ...
  const glbLinks = filled.filter(l =>
    l !== source && /\b(glb|gltf|stl|fbx|3d|مجسم)\b/i.test(l.label),
  );
  const paid = glbLinks.find(l => l.isPaid);
  const free = glbLinks.find(l => !l.isPaid);

  // لا ننسخ روابط PSD/PDF إلى glbUrl — حقل المجسم يُملأ يدوياً فقط أو من رابط وُسم 3D
  if (paid) {
    next.glbIsPaid = true;
