...
import {
  ...
  cameraPoseVectors,
  hasSavedOrbitPose,
  settingsWithAspectCompensation,
  ORBIT_DIST_MIN,
  ORBIT_DIST_MAX,
} from './gfxModel3d';
...
type GfxModelViewerProps = {
  ...
  /** خطوط توجيه متقاطعة في المنتصف (لوحة التحكم فقط) */
  centerGuide?: boolean;
...
function fitObjectToPivot(THREE: any, obj: any, pivot: any) {
...
export function GfxModelViewer({
  ...
  useEffect(() => {
    ...
    const disposeBundle = () => {
      ...
      try { (b as any).ro?.disconnect?.(); } catch { /* ignore */ }
    ...
    loaders
      .then(() => {
        ...
        if (useControls && THREE.OrbitControls) {
          ...
          // مجال رؤية أوسع — زوايا شبه كاملة أعلى/أسفل
        ...
        const loadBuffer = async (): Promise<ArrayBuffer> => {
          let pending = modelBufferCache.get(modelSrc);
          if (!pending) {
            pending = fetch(modelSrc)
              .then(async (res) => {
                if (!res.ok) {
              ...
              .catch((err) => {
                modelBufferCache.delete(modelSrc);
                throw err;
              });
            modelBufferCache.set(modelSrc, pending);
          }
          return pending.then((buf) => buf.slice(0));
        };
        ...
        const resize = () => {
          ...
          // إعادة تطبيق المنظور مع تعويض النسبة بعد تغيير حجم الحاوية
          const b = bundleRef.current;
          if (b && !b.userInteracting && !b.resetting && !(capturePose && poseLockedRef.current)) {
            const adjusted = settingsWithAspectCompensation(cfgRef.current, aspect);
            if (b.controls) syncControlsFromSettings(b.controls, camera, adjusted);
            else applyCameraPose(camera, adjusted);
          }
        };
  ...
  useEffect(() => {
    ...
    if (onPoseCaptureRef.current) {
      const host = canvasHostRef.current;
      const aspect = host && host.clientHeight
        ? host.clientWidth / host.clientHeight
        : undefined;
      const pose = capturePoseFromControls(b.camera, b.controls, aspect);
      poseLockedRef.current = true;
  ...
  useEffect(() => {
    ...
    const adjusted = settingsWithAspectCompensation(cfg, aspect);
    if (b.controls) {
      syncControlsFromSettings(b.controls, b.camera, adjusted);
    } else {
      applyCameraPose(b.camera, adjusted);
    }
  }, [cfg.viewPanX, cfg.viewPanY, cfg.viewPanZ, cfg.cameraDistance, cfg.orbitAzimuth, cfg.orbitPolar, cfg.orbitDistance, cfg.referenceAspect, status, capturePose]);
      ...
      {centerGuide && (
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 4 }}>
          ...
          <div style={{
            ...
            background: 'repeating-linear-gradient(to bottom, rgba(255,0,80,0.65) 0 6px, rgba(255,0,80,0) 6px 12px)',
          }} />
          {/* خط أفقي في المنتصف */}
          <div style={{
            position: 'absolute', left: 0, right: 0, top: '50%', height: 1,
            transform: 'translateY(-0.5px)',
            background: 'repeating-linear-gradient(to right, rgba(255,0,80,0.65) 0 6px, rgba(255,0,80,0) 6px 12px)',
          }} />
          {/* نقطة المركز */}
          <div style={{
            position: 'absolute', left: '50%', top: '50%', width: 8, height: 8,
            transform: 'translate(-50%, -50%)', borderRadius: '50%',
            border: '1px solid rgba(255,0,80,0.85)', background: 'rgba(255,0,80,0.15)',
          }} />
        </div>
      )}
