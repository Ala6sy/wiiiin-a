...
import {
  ...
  ml, pickML, flattenLibrary, LangKey, uid, ML,
} from './appData';
...
import { resolveImageSrc, resolveReportDisplay } from './mediaUrl';
...
function ReorderBtns({ index, total, onMove }: { index: number; total: number; onMove: (dir: -1 | 1) => void }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      <button type="button" className="btn-outline-sm" disabled={index === 0} onClick={() => onMove(-1)} title="للأعلى" style={{ padding: '2px 6px', fontSize: 10, lineHeight: 1 }}><i className="fa-solid fa-chevron-up" /></button>
      <button type="button" className="btn-outline-sm" disabled={index === total - 1} onClick={() => onMove(1)} title="للأسفل" style={{ padding: '2px 6px', fontSize: 10, lineHeight: 1 }}><i className="fa-solid fa-chevron-down" /></button>
    </div>
  );
}
...
function AgriAdmin({ data, onSave }: { data: AppData; onSave: (u: Partial<AppData>) => void }) {
  ...
  const renderBookPreviewCard = (b: AgriBook, i: number) => (
    <div key={b.id || i} className="book-grid-card" style={{ background: '#f5fbf5', border: '1px solid #c8e6c9', borderRadius: 8, overflow: 'hidden', fontSize: 11, position: 'relative' }}>
      ...
      <div style={{ padding: '5px 6px' }}>
        <div className="book-card-title" style={{ fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{pickML(b.title, lang)}</div>
        <div className="book-card-author" style={{ color: '#888' }}>{pickML(b.author, lang)}</div>
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 14, padding: '10px 14px', background: '#f0faf0', borderRadius: 10, border: '1px solid #c8e6c9', fontSize: 12, color: '#555' }}>
          ...
          {showArticleGrid && (
            <div style={{ background: '#f0f7f0', border: '1px solid #a5d6a7', borderRadius: 14, padding: 16, marginBottom: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 700, color: '#555', display: 'block', marginBottom: 6 }}>📱 عدد المقالات في السطر (جوال)</label>
                ...
                <div>
                  <label style={{ fontSize: 12, fontWeight: 700, color: '#555', display: 'block', marginBottom: 6 }}>🖥️ عدد المقالات في السطر (ويب)</label>
                  <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                    {[1,2,3,4,5,6].map(n => (
                ...
                <div>
                  <label style={{ fontSize: 12, fontWeight: 700, color: '#555', display: 'block', marginBottom: 6 }}>↔️ المسافة بين البطاقات: <span style={{ color: '#2a7a2a' }}>{articleGrid.gap}px</span></label>
                ...
                <div>
                  <label style={{ fontSize: 12, fontWeight: 700, color: '#555', display: 'block', marginBottom: 6 }}>📐 هامش الجانبين (جوال): <span style={{ color: '#2a7a2a' }}>{articleGrid.paddingMobile}px</span></label>
                ...
                <div>
                  <label style={{ fontSize: 12, fontWeight: 700, color: '#555', display: 'block', marginBottom: 6 }}>🖼️ ارتفاع صورة المقالة: <span style={{ color: '#2a7a2a' }}>{articleGrid.imgHeight}px</span></label>
                ...
                <div>
                  <label style={{ fontSize: 12, fontWeight: 700, color: '#555', display: 'block', marginBottom: 6 }}>📄 أسطر المقتطف: <span style={{ color: '#2a7a2a' }}>{articleGrid.excerptLines}</span></label>
          ...
          {editArticle ? (
            <div>
              ...
              <div className="form-group"><label>روابط الصور (متعددة)</label>
                <p style={{ fontSize: 11, color: '#666', margin: '0 0 8px' }}>يدعم روابط Google Drive — الصق رابط المشاركة /view كما هو من «نسخ الرابط»</p>
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                ...
                <div className="form-group">
                  ...
                  <p style={{ fontSize: 10, color: '#888', margin: '0 0 6px' }}>يدعم Google Drive — الصق رابط /view مباشرة</p>
              ...
              {showGridSettings && (
                <div style={{ background: '#f0f7f0', border: '1px solid #a5d6a7', borderRadius: 14, padding: 16, marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
                    ...
                    {/* Image Height */}
                    <div>
                      <label style={{ fontSize: 12, fontWeight: 700, color: '#555', display: 'block', marginBottom: 6 }}>📐 ارتفاع غلاف A4 (شاقولي): <span style={{ color: '#2a7a2a' }}>{bookGrid.imgHeight}px</span></label>
                    ...
                    {/* Card Width */}
                    <div>
                      <label style={{ fontSize: 12, fontWeight: 700, color: '#555', display: 'block', marginBottom: 6 }}>📏 عرض البطاقة: <span style={{ color: '#2a7a2a' }}>{bookGrid.cardWidth ?? 100}px</span></label>
                  ...
                  <div style={{ fontSize: 10, color: '#999', marginTop: 6, textAlign: 'center' }}>↑ معاينة تقريبية — تُطبَّق على الموقع حسب الجوال أو الويب</div>
                </div>
              )}
      ...
      {tab === 'media' && (
        <div>
            ...
            {videos.map(v => (
              <div key={v.id} style={{ border: '1px solid #c8e6c9', borderRadius: 10, padding: 12, background: '#f8fdf8' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                  <MlObjectTranslateButton small value={v.title}
                    onChange={title => saveVideos(videos.map(x => x.id === v.id ? { ...x, title } : x))} />
          ...
          <p style={{ fontSize: 12, color: '#666', marginBottom: 12 }}>ارفع صورة مصغرة أو الصق رابط Google Drive للتقرير PDF — يُولَّد الغلاف تلقائياً ويُفتح للقراءة داخل الموقع. اضبط الملف على «أي شخص لديه الرابط».</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: 12, marginBottom: 12 }}>
            {pubReports.map(r => (
              <div key={r.id} style={{ border: '1px solid #c8e6c9', borderRadius: 10, padding: 12, background: '#f8fdf8' }}>
                ...
                <input placeholder="رابط التقرير (Google Drive PDF أو أي رابط)" value={r.url} style={{ width: '100%', boxSizing: 'border-box', direction: 'ltr', padding: '7px 10px', border: '1px solid #ccc', borderRadius: 8, fontFamily: 'inherit', fontSize: 12.5, marginBottom: 4 }}
                  ...
                  onBlur={e => {
                    ...
                    if (disp.thumbSrc) {
                      savePubReports(pubReports.map(x => x.id === r.id ? { ...x, url, thumbnail: x.thumbnail || disp.thumbSrc } : x));
                    }
                  }} />
...
function GfxAdmin({ data, onSave }: { data: AppData; onSave: (u: Partial<AppData>) => void }) {
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 14, padding: '10px 14px', background: '#f0f4fa', borderRadius: 10, border: '1px solid #c0d4f0', fontSize: 12, color: '#555' }}>
      ...
      {tab === 'categories' && (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,170px) minmax(0,170px) minmax(0,1fr)', gap: 10 }}>
          {/* LEVEL 1 */}
          <div style={{ background: '#f0f4ff', borderRadius: 10, padding: 8, minWidth: 0, overflow: 'hidden' }}>
          ...
          {/* LEVEL 2 */}
          <div style={{ background: '#f5f0ff', borderRadius: 10, padding: 8, minWidth: 0, overflow: 'hidden' }}>
              ...
              editItem ? (
                <div style={{ overflowY: 'auto', maxHeight: '60vh' }}>
                  ...
                  <div className="form-group" style={{ marginBottom: 8 }}>
                    ...
                    <input type="url" value={editItem.mainImg} placeholder="https://drive.google.com/file/d/.../view" style={{ direction: 'ltr', fontSize: 11, color: '#003366', background: '#f8faff' }}
                      onChange={e => setEditItem({ ...editItem, mainImg: e.target.value })}
                      onBlur={e => { const n = normalizeImageUrlForStorage(e.target.value); if (n && n !== e.target.value) setEditItem({ ...editItem, mainImg: n }); }} />
                    {editItem.mainImg && <img src={resolveImageSrc(editItem.mainImg)} alt="" style={{ width: '100%', height: 80, objectFit: 'cover', borderRadius: 6, marginTop: 4 }} />}
                  ...
                  {/* Original source file (PSD / DWG / C4D …) */}
                  <div style={{ marginBottom: 10, background: '#f5f0ff', borderRadius: 10, padding: '10px 12px', border: '1px solid #d4c8f5' }}>
                    <div style={{ fontWeight: 700, fontSize: 11, color: '#4a2a8a', marginBottom: 8 }}>
                      <i className="fa-solid fa-file-arrow-down" /> ملف التصميم الأصلي (PSD / AutoCAD / C4D …)
                    </div>
                    <p style={{ fontSize: 10, color: '#777', margin: '0 0 8px', lineHeight: 1.5 }}>
                      ارفع الملف على Google Drive واضبط «أي شخص لديه الرابط» ثم الصق الرابط هنا.
                    </p>
                    <div className="form-group" style={{ marginBottom: 6 }}>
                      <label style={{ fontSize: 10, color: '#555' }}>رابط Google Drive للملف الأصلي</label>
                      <input type="url" value={editItem.sourceFileUrl || ''} placeholder="https://drive.google.com/file/d/.../view"
                        style={{ direction: 'ltr', fontSize: 11, color: '#4a2a8a', background: '#fff' }}
                        onChange={e => setEditItem({ ...editItem, sourceFileUrl: e.target.value })} />
                    </div>
                    {editItem.sourceFileUrl && (
                      <>
                        <div className="form-group" style={{ marginBottom: 6 }}>
                          <label style={{ fontSize: 10, color: '#555' }}>نوع الملف (يظهر للزائر) — مثال: PSD، DWG، C4D</label>
                          <input type="text" value={editItem.sourceFileLabel || ''} placeholder="PSD"
                            style={{ fontSize: 11 }}
                            onChange={e => setEditItem({ ...editItem, sourceFileLabel: e.target.value })} />
                        </div>
                        <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, marginBottom: 8, cursor: 'pointer', fontWeight: 600, color: editItem.sourceFileVisible !== false ? '#2a7a2a' : '#999' }}>
                          <input type="checkbox" checked={editItem.sourceFileVisible !== false}
                            onChange={e => setEditItem({ ...editItem, sourceFileVisible: e.target.checked })} />
                          {editItem.sourceFileVisible !== false ? '👁️ زر التنزيل ظاهر للزوار' : '🚫 مخفي — لا يظهر زر التنزيل'}
                        </label>
                        <div className="form-group" style={{ marginBottom: 0 }}>
                          <label style={{ fontSize: 10, color: '#555' }}>
                            <i className="fa-solid fa-lock" /> كلمة سر التنزيل (اختياري — من لوحة التحكم فقط)
                          </label>
                          <input type="text" value={editItem.sourceFilePassword || ''} placeholder="اتركه فارغاً = تنزيل مباشر بدون كلمة سر"
                            style={{ direction: 'ltr', fontSize: 11, fontFamily: 'monospace' }}
                            onChange={e => setEditItem({ ...editItem, sourceFilePassword: e.target.value })} />
                          <p style={{ fontSize: 9.5, color: '#999', margin: '4px 0 0' }}>يمكنك تغييرها أو حذفها في أي وقت ثم حفظ المشروع.</p>
                        </div>
                      </>
                    )}
                  </div>
                  ...
                  {selSub.items.map(item => {
                    ...
                    return (
                      <button key={item.id} onClick={() => setEditItem(item)}
                        ...
                        {item.mainImg ? <img src={resolveImageSrc(item.mainImg)} alt="" style={{ width: 40, height: 36, objectFit: 'cover', borderRadius: 4, flexShrink: 0 }} /> : <div style={{ width: 40, height: 36, background: '#eee', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><i className="fa-solid fa-image" style={{ fontSize: 12 }} /></div>}
                        ...
                        {item.sourceFileUrl && item.sourceFileVisible !== false && (
                          <span style={{ fontSize: 9, background: '#4a2a8a', color: '#fff', borderRadius: 4, padding: '1px 4px', flexShrink: 0 }}>
                            {item.sourceFilePassword ? '🔒' : '📁'} ملف
                          </span>
                        )}
                      </button>
                    );
      ...
      {tab === 'grid' && (
        <div style={{ background: '#f0f4ff', border: '1px solid #aac4ee', borderRadius: 14, padding: 20 }}>
          ...
          <div style={{ fontSize: 10, color: '#999', marginTop: 6, textAlign: 'center' }}>↑ تُطبَّق على الموقع — ويب للشاشات العريضة، جوال لعرض ≤768px</div>
        </div>
      )}
                ...
                {allItems.map(({ item, catName, subName }) => {
                  ...
                  return (
                    <div key={item.id} style={{ background: '#fff', border: '1px solid #f0d8a0', borderRadius: 10, padding: '10px 14px', display: 'flex', gap: 10, alignItems: 'center' }}>
                      {item.mainImg ? <img src={resolveImageSrc(item.mainImg)} alt="" style={{ width: 52, height: 44, objectFit: 'cover', borderRadius: 6, flexShrink: 0 }} /> : <div style={{ width: 52, height: 44, background: '#eee', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><i className="fa-solid fa-image" style={{ color: '#aaa' }} /></div>}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            {projects.map((p, i) => (
              <button key={p.id} onClick={() => { setSel(i); setDraft({ ...p, images: [...p.images] }); }}
                ...
                {p.mainImg && <img src={resolveImageSrc(p.mainImg)} style={{ width: 32, height: 32, objectFit: 'cover', borderRadius: 6, flexShrink: 0 }} />}
        ...
        <div>
          {draft !== null && sel !== null ? (
            <div>
              ...
              {draft.mainImg && (
                <div style={{ marginBottom: 14 }}>
                  <img src={resolveImageSrc(draft.mainImg)} style={{ height: 100, borderRadius: 10, objectFit: 'cover', width: '100%' }} />
                </div>
              )}
