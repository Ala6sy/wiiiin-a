...
function AgriAdmin({ data, onSave }: { data: AppData; onSave: (u: Partial<AppData>) => void }) {
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 14, padding: '10px 14px', background: '#f0faf0', borderRadius: 10, border: '1px solid #c8e6c9', fontSize: 12, color: '#223344' }}>
      ...
      {tab === 'media' && (
        <div>
          ...
          <p style={{ fontSize: 12, color: '#334455', marginBottom: 12 }}>الصق رابط الفيديو (يوتيوب / جوجل درايف / فيميو / رابط مباشر). يظهر في أعلى تبويب "تحليل التربة والنبات". لا يتم تخزين الفيديو على الموقع — فقط الرابط.</p>
          ...
          <p style={{ fontSize: 12, color: '#334455', marginBottom: 12 }}>ارفع صورة مصغرة أو الصق رابط Google Drive للتقرير PDF — يُولَّد الغلاف تلقائياً ويُفتح للقراءة داخل الموقع. اضبط الملف على «أي شخص لديه الرابط».</p>
            ...
            {pubReports.map(r => (
              <div key={r.id} style={{ border: '1px solid #c8e6c9', borderRadius: 10, padding: 12, background: '#f8fdf8' }}>
                <div onClick={() => reportThumbRefs.current[r.id]?.click()} style={{ cursor: 'pointer', height: 120, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px dashed #bbb', borderRadius: 8, background: '#fff', marginBottom: 8, overflow: 'hidden' }}>
                  {r.thumbnail ? <img src={r.thumbnail} alt="" style={{ maxWidth: '100%', maxHeight: 120, objectFit: 'contain' }} /> : <span style={{ color: '#556677', fontSize: 12 }}><i className="fa-solid fa-upload" /> صورة مصغرة</span>}
                </div>
...
function GfxAdmin({ data, onSave }: { data: AppData; onSave: (u: Partial<AppData>) => void }) {
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 14, padding: '10px 14px', background: '#f0f4fa', borderRadius: 10, border: '1px solid #c0d4f0', fontSize: 12, color: '#223344' }}>
      ...
      {tab === 'categories' && (
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 6, marginBottom: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          <span style={{ fontSize: 10, color: '#334455' }}>كل الفروع في التصنيف الحالي:</span>
          {[1, 5, 10].map(n => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,170px) minmax(0,170px) minmax(0,1fr)', gap: 10 }}>
          ...
          {/* LEVEL 3 */}
          <div style={{ background: '#fffaf0', borderRadius: 10, padding: 8, minWidth: 0, overflow: 'hidden' }}>
            ...
            {selSub ? (
              editItem ? (
                <div style={{ overflowY: 'auto', maxHeight: '60vh' }}>
                  ...
                  {/* 3D model GLB / STL */}
                  {(() => {
                    const modelDriveUrl = (editItem.glbUrl || editItem.glbFreeUrl || '').trim();
                    return (
                      <>
                        <div className="form-group" style={{ marginBottom: 8 }}>
                          <label style={{ fontSize: 10, color: '#223344' }}>رابط Google Drive لملف GLB / FBX / STL (رابط /view)</label>
                          <input type="url" value={editItem.glbUrl || ''} placeholder="https://drive.google.com/file/d/XXXX/view?usp=sharing" style={{ direction: 'ltr', fontSize: 11, color: '#003366', background: '#fff' }}
                            onChange={e => setEditItem({ ...editItem, glbUrl: e.target.value })} />
                          <p style={{ fontSize: 9.5, color: '#556677', margin: '4px 0 0' }}>الصق رابط /view هنا — أو في حقل التنزيل المجاني بالأسفل (يُستخدم تلقائياً للمعاينة).</p>
                        </div>
                        {modelDriveUrl && (
                          <GfxModel3dAdmin
                            url={modelDriveUrl}
                  ...
                  {selSub.items.map((item, pi) => {
                    ...
                    return (
                      ...
                      <div style={{
                        ...
                        background: '#fff', border: '1px solid #ede', borderRadius: 7, padding: '4px 5px', overflow: 'hidden',
                      }}>
                      <button type="button" onClick={() => openGfxProject(item.id)}
                        style={{ flex: 1, minWidth: 0, display: 'flex', gap: 6, alignItems: 'center', background: 'none', border: 'none', borderRadius: 4, padding: '2px 4px', cursor: 'pointer', textAlign: 'right' }}>
                        {item.mainImg ? <img src={resolveImageSrc(item.mainImg)} alt="" style={{ width: 40, height: 36, objectFit: 'cover', borderRadius: 4, flexShrink: 0 }} /> : <div style={{ width: 40, height: 36, background: '#eee', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><i className="fa-solid fa-image" style={{ fontSize: 12 }} /></div>}
                        ...
                        {getGfxDownloadLinks(item).filter(l => l.visible !== false).length > 0 && (
                          <span style={{ fontSize: 9, background: '#006644', color: '#fff', borderRadius: 4, padding: '1px 4px', flexShrink: 0 }}>
...
function SiteSettingsAdmin({
  data, onApply, onPersist, serverConnected, serverSyncing, onServerConnect, onServerSync, onServerDisconnect,
}: {
  data: AppData;
    ...
    <div>
        ...
        <div style={{
          ...
          {syncStatus !== 'saving' && (
            <button type="button" className="btn-outline-sm" onClick={() => void flushPersist()} style={{ fontSize: 11 }}>
              <i className="fa-solid fa-cloud-arrow-up" /> حفظ ورفع الآن
            </button>
          )}
        </div>
      )}
      ...
      <div style={{ background: '#f8f9ff', border: '1px solid #dde', borderRadius: 10, padding: 14, marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 4, flexWrap: 'wrap' }}>
          <div style={{ fontWeight: 700, color: '#003366' }}><i className="fa-solid fa-palette" /> ألوان الموقع والقوائم</div>
          <button
            type="button"
        ...
        <div style={{
          ...
          <span style={{ fontSize: 12, color: '#556677', fontWeight: 600 }}>معاينة الأزرار:</span>
        ...
        <div className="form-group">
          <label style={{ color: '#003366', fontWeight: 700 }}>درجة شفافية القوائم الزجاجية ({Math.round((settings.glassOpacity ?? 0.5) * 100)}%)</label>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#556677' }}>
      ...
      <div style={{ background: '#f8f9ff', border: '1px solid #dde', borderRadius: 10, padding: 14, marginBottom: 14 }}>
        <div style={{ fontWeight: 700, marginBottom: 4, color: '#003366' }}><i className="fa-solid fa-font" /> الخط والنصوص — الموقع بالكامل</div>
        <div style={{ fontSize: 12, color: '#445566', marginBottom: 12, lineHeight: 1.7 }}>
...
/* ══════════════════════════════════════════════════════
   MAIN EXPORT
══════════════════════════════════════════════════════ */
export function ContentAdmin({ mode, data, onSave, onSiteApply, onSitePersist, serverConnected, serverSyncing, onServerConnect, onServerSync, onServerDisconnect }: Props) {
