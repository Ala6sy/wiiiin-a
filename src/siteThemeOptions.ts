...
/** لون خط مقروء على خلفية معيّنة */
export function contrastTextForBg(bgHex: string, darkText = '#0a1a2e', lightText = '#ffffff'): string {
  return isLightBackground(bgHex) ? darkText : lightText;
}

/** يُبقي اللون إن كان مقروءاً، وإلا يختار تلقائياً */
export function pickReadableText(textHex: string, bgHex: string): string {
  const text = expandHex(textHex);
  const bg = expandHex(bgHex);
  if (!text || !bg) return textHex || contrastTextForBg(bgHex);
  const ratio = (Math.max(relativeLuminance(text), relativeLuminance(bg)) + 0.05)
    / (Math.min(relativeLuminance(text), relativeLuminance(bg)) + 0.05);
  if (ratio >= 3.2) return text;
