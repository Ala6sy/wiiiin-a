...
import { resolveVideoEmbedSrc, resolveImageSrc, resolveReportDisplay } from './mediaUrl';
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 12 }}>
            {reports.map(r => {
              ...
              const thumbSrc = disp.thumbSrc || resolveImageSrc(r.thumbnail);
              const openReport = (e: React.MouseEvent) => {
                if (!r.url && !thumbSrc) return;
                e.preventDefault();
              ...
              const inner = (
                <>
                  ...
                  <div style={{ padding: '8px 10px' }}>
                    ...
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11.5, fontWeight: 700, color: (r.url || thumbSrc) ? 'var(--navy)' : 'var(--muted)' }}>
                      <i className={`fa-solid ${disp.useModal ? 'fa-file-pdf' : 'fa-up-right-from-square'}`} /> {disp.useModal ? (lang === 'ar' ? 'قراءة PDF' : lang === 'de' ? 'PDF lesen' : 'Read PDF') : L(LBL.viewReport)}
                    </span>
                  </div>
                </>
              );
              const cardStyle: React.CSSProperties = { background: 'var(--glass)', border: '1px solid var(--glass-border)', borderRadius: 12, overflow: 'hidden', textDecoration: 'none', display: 'block', cursor: (r.url || thumbSrc) ? 'pointer' : 'default' };
              return (r.url || thumbSrc)
                ? <a key={r.id} href={disp.useModal ? '#' : (r.url || thumbSrc)} onClick={openReport} style={cardStyle}>{inner}</a>
