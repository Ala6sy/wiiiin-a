...
CREATE TABLE IF NOT EXISTS `site_settings` (
  ...
  `name_display`          ENUM('text','handwriting','logo') NOT NULL DEFAULT 'text',
  `name_logo`             TEXT             DEFAULT NULL,
  `name_shimmer`          TINYINT(1)       NOT NULL DEFAULT 1,
  `name_logo_color`       VARCHAR(30)      DEFAULT '#ffffff',
  `name_shimmer_speed`    FLOAT            DEFAULT 3.2,
  `name_shimmer_color`    VARCHAR(30)      DEFAULT '#00ccff',
  `name_shimmer_angle`    SMALLINT         DEFAULT 90,
  `name_shimmer_motion`   TINYINT(1)       NOT NULL DEFAULT 1,
  `name_shimmer_direction` ENUM('rtl','ltr') NOT NULL DEFAULT 'rtl',
  `name_shimmer_width`    FLOAT            DEFAULT 0.08,
...
CREATE TABLE IF NOT EXISTS `gfx_items` (
  ...
  `position_index` SMALLINT UNSIGNED DEFAULT 0,
  `item_meta`      JSON        DEFAULT NULL COMMENT 'GLB، ملف مصدر، علامة مائية…',
  `created_at`     DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `cv_sections` (
  ...
  `skill_bar_color`   VARCHAR(12)  DEFAULT NULL,
  `sort_order`        SMALLINT UNSIGNED DEFAULT 0,
  PRIMARY KEY (`id`),
...
-- ── site_settings: عرض الاسم (خط يد / شعار / إضاءة) ────────
ALTER TABLE `site_settings`
  ADD COLUMN IF NOT EXISTS `name_display` ENUM('text','handwriting','logo') NOT NULL DEFAULT 'text' AFTER `site_bio`,
  ADD COLUMN IF NOT EXISTS `name_logo`    TEXT         DEFAULT NULL                    AFTER `name_display`,
  ADD COLUMN IF NOT EXISTS `name_shimmer` TINYINT(1)   NOT NULL DEFAULT 1              AFTER `name_logo`;

ALTER TABLE `site_settings`
  ADD COLUMN IF NOT EXISTS `name_logo_color`    VARCHAR(30) DEFAULT '#ffffff' AFTER `name_shimmer`,
  ADD COLUMN IF NOT EXISTS `name_shimmer_speed` FLOAT       DEFAULT 3.2     AFTER `name_logo_color`;

ALTER TABLE `site_settings`
  ADD COLUMN IF NOT EXISTS `name_shimmer_color` VARCHAR(30) DEFAULT '#00ccff' AFTER `name_shimmer_speed`;

ALTER TABLE `site_settings`
  ADD COLUMN IF NOT EXISTS `name_shimmer_angle` SMALLINT DEFAULT 90 AFTER `name_shimmer_color`;

ALTER TABLE `site_settings`
  ADD COLUMN IF NOT EXISTS `name_shimmer_motion` TINYINT(1) NOT NULL DEFAULT 1 AFTER `name_shimmer_angle`;

ALTER TABLE `site_settings`
  ADD COLUMN IF NOT EXISTS `name_shimmer_direction` ENUM('rtl','ltr') NOT NULL DEFAULT 'rtl' AFTER `name_shimmer_motion`;

ALTER TABLE `site_settings`
  ADD COLUMN IF NOT EXISTS `name_shimmer_width` FLOAT DEFAULT 0.08 AFTER `name_shimmer_direction`;
...
ALTER TABLE `cv_sections`
  ADD COLUMN IF NOT EXISTS `skill_bar_color` VARCHAR(12) DEFAULT NULL AFTER `skill_ids`;

-- ── cv_docs: إعدادات الترويسة/التذييل متعددة الصفحات ──
ALTER TABLE `cv_docs`
  ADD COLUMN IF NOT EXISTS `page_layout` JSON DEFAULT NULL AFTER `qr_credentials`;

-- ── skills: توسيع icon لدعم روابط الصور (كانت VARCHAR(120) فقط) ──
ALTER TABLE `skills`
  MODIFY COLUMN `icon` TEXT DEFAULT NULL;
