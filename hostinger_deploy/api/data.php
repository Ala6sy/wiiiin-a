...
/** تنظيف قيم ML التالفة (مثل نص "Array" من PHP القديم) */
function cleanMLPart($s): string
{
    $t = trim((string)($s ?? ''));
    if ($t === '' || $t === 'Array' || $t === '[object Object]') return '';
    return $t;
}

/** قراءة حقل ML من DB (JSON أو نص قديم) */
function readMLField($val, string $defaultAr = ''): array
{
    if ($val === null || $val === '' || $val === 'Array' || $val === '[object Object]') {
        return ['ar' => $defaultAr, 'en' => '', 'de' => ''];
    }
    if (is_array($val)) {
        return [
            'ar' => cleanMLPart($val['ar'] ?? '') ?: $defaultAr,
            'en' => cleanMLPart($val['en'] ?? ''),
            'de' => cleanMLPart($val['de'] ?? ''),
        ];
    }
    if (!is_string($val)) {
        return ['ar' => $defaultAr, 'en' => '', 'de' => ''];
    ...
    $decoded = json_decode($val, true);
    if (is_array($decoded) && (array_key_exists('ar', $decoded) || array_key_exists('en', $decoded) || array_key_exists('de', $decoded))) {
        return [
            'ar' => cleanMLPart($decoded['ar'] ?? '') ?: $defaultAr,
            'en' => cleanMLPart($decoded['en'] ?? ''),
            'de' => cleanMLPart($decoded['de'] ?? ''),
        ];
    }
    return ['ar' => cleanMLPart($val) ?: $defaultAr, 'en' => '', 'de' => ''];
}

/** حفظ حقل ML في DB كـ JSON */
function writeMLField($val, string $fallback = ''): string
{
    if (is_array($val)) {
        return je([
            'ar' => cleanMLPart($val['ar'] ?? '') ?: $fallback,
            'en' => cleanMLPart($val['en'] ?? ''),
            'de' => cleanMLPart($val['de'] ?? ''),
        ]);
    }
    if (is_string($val) && $val !== '') {
        return je(['ar' => $val, 'en' => $val, 'de' => $val]);
    }
    return je(['ar' => $fallback, 'en' => '', 'de' => '']);
}
...
function buildAppData(PDO $db): array
{
    ...
    return [
        ...
        // معلومات الموقع
        'name'                 => readMLField($ss['site_name'] ?? null, 'م. علاء أحمد المصري'),
        'bio'                  => readMLField($ss['site_bio'] ?? null, ''),
        'nameDisplay'          => $ss['name_display'] ?? 'text',
        'nameLogo'             => $ss['name_logo']    ?? '',
        'nameShimmer'          => (bool)($ss['name_shimmer'] ?? 1),
        'nameLogoColor'        => $ss['name_logo_color'] ?? '#ffffff',
        'nameShimmerSpeed'     => (float)($ss['name_shimmer_speed'] ?? 3.2),
        'nameShimmerColor'     => $ss['name_shimmer_color'] ?? '#00ccff',
        'nameShimmerAngle'     => (int)($ss['name_shimmer_angle'] ?? 90),
        'nameShimmerMotion'    => (bool)($ss['name_shimmer_motion'] ?? 1),
        'nameShimmerDirection' => in_array($ss['name_shimmer_direction'] ?? 'rtl', ['rtl', 'ltr'], true)
          ? ($ss['name_shimmer_direction'] ?? 'rtl') : 'rtl',
        'nameShimmerWidth'     => (float)($ss['name_shimmer_width'] ?? 0.08),
        'currency'             => $rt['currency']  ?? '',
        ...
        'showAgriCv'           => (bool)($ss['show_agri_cv']           ?? false),
        'showDesignerCv'       => (bool)($ss['show_designer_cv']       ?? false),
        'agriCvPlacements'     => (function () use ($ss) {
            $gs = jd($ss['grid_settings'] ?? null, []);
            $cp = is_array($gs['cvPlacements'] ?? null) ? $gs['cvPlacements'] : [];
            return is_array($cp['agri'] ?? null) ? $cp['agri'] : null;
        })(),
        'designCvPlacements'   => (function () use ($ss) {
            $gs = jd($ss['grid_settings'] ?? null, []);
            $cp = is_array($gs['cvPlacements'] ?? null) ? $gs['cvPlacements'] : [];
            return is_array($cp['design'] ?? null) ? $cp['design'] : null;
        })(),
        'agriCats'             => jd($ss['agri_cats'] ?? null, []),
        'libraryView'          => $ss['library_view'] ?? 'tree',
        'bookGridSettings'     => ($gs = jd($ss['grid_settings'] ?? null, []))['bookGridSettings'] ?? null,
...
// ─── بيانات إضافية لعناصر التصاميم (GLB، ملف مصدر، علامة مائية…) ───
...
function gfxItemFromMeta(array $base, ?string $metaJson): array
{
    $meta = jd($metaJson, []);
    if (!is_array($meta)) $meta = [];
    foreach (gfxItemMetaKeys() as $k) {
...
function gfxItemMetaToJson(array $item): string
{
    $meta = [];
    foreach (gfxItemMetaKeys() as $k) {
        if (!array_key_exists($k, $item)) continue;
        $v = $item[$k];
    ...
    if (($item['sourceFileVisible'] ?? true) === false) {
        $meta['sourceFileVisible'] = false;
    }
    return je($meta);
}

function ensureGfxItemMetaColumn(PDO $db): void
{
    static $done = false;
    if ($done) return;
    $done = true;
    try {
        $col = $db->query("SHOW COLUMNS FROM gfx_items LIKE 'item_meta'")->fetch();
        if (!$col) {
            $db->exec("ALTER TABLE gfx_items ADD COLUMN item_meta JSON DEFAULT NULL AFTER position_index");
        }
    } catch (\Throwable $e) {
        // يُكمل بدون العمود — يُفقد الـ meta عند المزامنة حتى يُضاف العمود يدوياً
    }
}
...
function buildGfxCategories(PDO $db): array
{
    ...
    foreach ($items as $item) {
        ...
        $itemsBySubId[$sid][] = gfxItemFromMeta([
            'id'           => $item['id'],
...
function buildCvDocs(PDO $db): array
{
    ...
    foreach ($sections as $s) {
        ...
        $secsByDocId[$did][] = [
            ...
            'skillIds'        => jd($s['skill_ids'] ?? null, []),
            'skillBarColor'   => $s['skill_bar_color'] ?? null,
        ];
    }
    ...
    foreach ($docs as $d) {
        $result[] = [
            'id'            => $d['id'],
            'name'          => readMLField($d['name'] ?? null, 'CV'),
        ...
        $pl = jd($d['page_layout'] ?? null, []);
        if ($pl) {
            $idx = count($result) - 1;
            if (isset($pl['headerHeightMm'])) $result[$idx]['headerHeightMm'] = (float)$pl['headerHeightMm'];
            if (isset($pl['footerHeightMm'])) $result[$idx]['footerHeightMm'] = (float)$pl['footerHeightMm'];
            if (isset($pl['headerPhotoSize'])) $result[$idx]['headerPhotoSize'] = (int)$pl['headerPhotoSize'];
            if (isset($pl['continuedPages'])) $result[$idx]['continuedPages'] = $pl['continuedPages'];
            if (isset($pl['pageOverrides'])) $result[$idx]['pageOverrides'] = $pl['pageOverrides'];
            if (isset($pl['extraBlankPages'])) $result[$idx]['extraBlankPages'] = (int)$pl['extraBlankPages'];
            if (isset($pl['pageSequence']) && is_array($pl['pageSequence'])) $result[$idx]['pageSequence'] = $pl['pageSequence'];
            if (isset($pl['footerLayout'])) $result[$idx]['footerLayout'] = $pl['footerLayout'];
            if (isset($pl['footerBarWidthPct'])) $result[$idx]['footerBarWidthPct'] = (float)$pl['footerBarWidthPct'];
            if (isset($pl['footerTextInside'])) $result[$idx]['footerTextInside'] = (bool)$pl['footerTextInside'];
            if (!empty($pl['pdfFiles']) && is_array($pl['pdfFiles'])) {
                $result[$idx]['pdfFiles'] = $pl['pdfFiles'];
            }
            if (!empty($pl['typography']) && is_array($pl['typography'])) {
                $result[$idx]['typography'] = $pl['typography'];
            }
        }
    }
    return $result;
...
function syncSiteSettings(PDO $db, array $d): void
{
    ...
    $db->prepare("
        UPDATE site_settings SET
          ...
          name_display         = ?, name_logo         = ?, name_shimmer      = ?,
    ...
    ")->execute([
        ...
        je($ss['socialLinks'] ?? []),
        je($ss['navItems']    ?? []),
        writeMLField($d['name'] ?? null, 'م. علاء أحمد المصري'),
        writeMLField($d['bio'] ?? null, ''),
        je($d['agriCats']     ?? []),
        ($d['aiDiagnosticsEnabled'] ?? false) ? 1 : 0,
        ($d['showAgriCv']     ?? false) ? 1 : 0,
        ($d['showDesignerCv'] ?? false) ? 1 : 0,
        $d['watermarkImg']    ?? '',
        (float)($d['watermarkOpacity'] ?? 0.15),
        $d['libraryView']     ?? 'tree',
        in_array($d['nameDisplay'] ?? 'text', ['text', 'handwriting', 'logo'], true) ? $d['nameDisplay'] : 'text',
        $d['nameLogo']        ?? '',
        ($d['nameShimmer'] ?? true) ? 1 : 0,
        $d['nameLogoColor']   ?? '#ffffff',
        (float)($d['nameShimmerSpeed'] ?? 3.2),
        $d['nameShimmerColor'] ?? '#00ccff',
        (int)($d['nameShimmerAngle'] ?? 90),
        ($d['nameShimmerMotion'] ?? true) ? 1 : 0,
        in_array($d['nameShimmerDirection'] ?? 'rtl', ['rtl', 'ltr'], true) ? ($d['nameShimmerDirection'] ?? 'rtl') : 'rtl',
        (float)($d['nameShimmerWidth'] ?? 0.08),
        je([
            'bookGridSettings'    => $d['bookGridSettings'] ?? null,
    ...
    $stmtItem = $db->prepare("
        INSERT INTO gfx_items
          (id, subcategory_id, title, description, main_img, images, video_url,
           used_skill_ids, cv_is_featured, cv_img_size, cv_show_desc, cv_show_tools, position_index, item_meta)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ");
...
function syncCvDocs(PDO $db, array $docs): void
{
    ...
    $stmtDoc = $db->prepare("
        INSERT INTO cv_docs
          (id, name, removable, accent, icon, photo, full_name, subtitle, since,
           show_in_about, global_color, footer_bg_color, footer_text, sidebar_docs, qr_credentials, page_layout, sort_order)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE
          ...
          qr_credentials=VALUES(qr_credentials), page_layout=VALUES(page_layout), sort_order=VALUES(sort_order)
    ");
    $stmtSec = $db->prepare("
        INSERT INTO cv_sections
          (id, cv_doc_id, kind, title, column_pos, visible, entries, tags,
           ...
           gallery_layout, img_height, page_break_before, skill_ids, skill_bar_color, sort_order)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE
          ...
          page_break_before=VALUES(page_break_before), skill_ids=VALUES(skill_ids),
          skill_bar_color=VALUES(skill_bar_color), sort_order=VALUES(sort_order)
    ");
    ...
    foreach ($docs as $di => $doc) {
        ...
        $stmtDoc->execute([
            ...
            je([
                'headerHeightMm'  => $doc['headerHeightMm'] ?? null,
                'footerHeightMm'  => $doc['footerHeightMm'] ?? null,
                'headerPhotoSize' => $doc['headerPhotoSize'] ?? null,
                'continuedPages'  => $doc['continuedPages'] ?? null,
                'pageOverrides'   => $doc['pageOverrides'] ?? null,
                'extraBlankPages' => isset($doc['extraBlankPages']) ? (int)$doc['extraBlankPages'] : null,
                'pageSequence'    => $doc['pageSequence'] ?? null,
                'footerLayout'    => $doc['footerLayout'] ?? null,
                'footerBarWidthPct' => $doc['footerBarWidthPct'] ?? null,
                'footerTextInside'  => $doc['footerTextInside'] ?? null,
                'pdfFiles'          => $doc['pdfFiles'] ?? null,
